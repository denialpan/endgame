...
public class MobAnnihilatorItemRenderer extends BlockEntityWithoutLevelRenderer {
    public static final MobAnnihilatorItemRenderer INSTANCE = new MobAnnihilatorItemRenderer();
    private static final ResourceLocation TEXTURE = ResourceLocation.fromNamespaceAndPath(dddsendgame.MODID, "textures/item/galaxy_mob_annihilator.png");
    ...
    private static void renderOriginalGeneratedModel(ItemStack stack, PoseStack poseStack, MultiBufferSource buffer, int packedLight, int packedOverlay) {
        BakedModel model = MobAnnihilatorModel.originalModel();
        if (model == null) {
            return;
        }
    ...
    private static PixelMasks pixelMasks() {
        ...
        if (resource.isPresent()) {
            try (InputStream stream = resource.get().open(); NativeImage image = NativeImage.read(stream)) {
                ...
                dddsendgame.LOGGER.warn("Unable to load mob annihilator texture masks", exception);
            }
        }
