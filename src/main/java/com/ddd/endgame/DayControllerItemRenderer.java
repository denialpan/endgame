...
public class DayControllerItemRenderer extends BlockEntityWithoutLevelRenderer {
    public static final DayControllerItemRenderer INSTANCE = new DayControllerItemRenderer();
    private static final ResourceLocation TEXTURE = ResourceLocation.fromNamespaceAndPath(dddsendgame.MODID, "textures/item/galaxy_day_controller.png");
    ...
    private static void renderOriginalGeneratedModel(ItemStack stack, PoseStack poseStack, MultiBufferSource buffer, int packedLight, int packedOverlay) {
        BakedModel model = DayControllerModel.originalModel();
        if (model == null) {
            return;
        }
    ...
    private static PixelMasks pixelMasks() {
        ...
        if (resource.isPresent()) {
            try (InputStream stream = resource.get().open(); NativeImage image = NativeImage.read(stream)) {
                ...
                dddsendgame.LOGGER.warn("Unable to load day controller texture masks", exception);
            }
        }
