...
public class WeatherCycleItem extends Item {
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltipComponents, TooltipFlag tooltipFlag) {
        tooltipComponents.add(Component.translatable("item.xavitia.weather_cycler.tooltip").withStyle(ChatFormatting.DARK_GRAY, ChatFormatting.ITALIC));
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand usedHand) {
        ...
        if (thundering) {
            ...
            player.displayClientMessage(Component.translatable("message.xavitia.weather.clear"), true);
        } else if (raining) {
            serverLevel.setWeatherParameters(0, WEATHER_DURATION, true, true);
            player.displayClientMessage(Component.translatable("message.xavitia.weather.thunder"), true);
        } else {
            serverLevel.setWeatherParameters(0, WEATHER_DURATION, true, false);
            player.displayClientMessage(Component.translatable("message.xavitia.weather.rain"), true);
        }
