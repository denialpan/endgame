...
public class RealityShifterItemRenderer extends BlockEntityWithoutLevelRenderer {
    public static final RealityShifterItemRenderer INSTANCE = new RealityShifterItemRenderer();
    private static final ResourceLocation TEXTURE = ResourceLocation.fromNamespaceAndPath(dddsendgame.MODID, "textures/item/galaxy_reality_shifter.png");
    ...
    public void renderByItem(ItemStack stack, ItemDisplayContext displayContext, PoseStack poseStack, MultiBufferSource buffer, int packedLight, int packedOverlay) {
        ...
        boolean[][] stencil = pixelMasks().stencil();
        if (displayContext == ItemDisplayContext.GROUND) {
            EndgamePortalBlockEntityRenderer.registerPixelWindowMask(poseStack.last().pose(), stencil, MASK_SIZE, FRONT_Z, BACK_Z);
            RenderOptimizationCompat.afterSkyboxItemRender(displayContext);
            return;
        }

        renderStencilWindow(displayContext, poseStack, stencil);
        RenderOptimizationCompat.afterSkyboxItemRender(displayContext);
    }
    ...
    private static void renderOriginalGeneratedModel(ItemStack stack, PoseStack poseStack, MultiBufferSource buffer, int packedLight, int packedOverlay) {
        BakedModel model = RealityShifterModel.originalModel();
        if (model == null) {
            return;
        }
    ...
    private static void renderStencilWindow(ItemDisplayContext displayContext, PoseStack poseStack, boolean[][] stencil) {
    ...
    private static PixelMasks pixelMasks() {
        ...
        if (resource.isPresent()) {
            try (InputStream stream = resource.get().open(); NativeImage image = NativeImage.read(stream)) {
                ...
                dddsendgame.LOGGER.warn("Unable to load reality shifter texture masks", exception);
            }
        }
